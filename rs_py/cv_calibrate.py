...
'''
TAKEN FROM: opencv/samples/python/calibration
...
def main():
    ...
    if not img_mask:
        img_mask = '/usr/src/opencv/samples/data/left??.jpg'  # default
    else:
        img_mask = img_mask[0]
    ...
    def processImage(fn):
        ...
        assert w == img.shape[1] and h == img.shape[0], ("size: %d x %d ... " % (img.shape[1], img.shape[0]))  # noqa
